...
| TC-01 | phishing-login | /pages/fake-login.html | `suspicious` | `ml`(phishing), `impersonation`, `pii_harvest`(has_login_form) | suspicious | 5 | `"nonstandard_port": "port=18080"`<br><br>(ExfiltrationDetected + MaxSensitivity = "high"이면 스코어링 3점 적용됨에도 evidence에 없는 이유는? : exfiltration_detected가 false 라서 스코어링에 적용 x) | ML off·브랜드 미매칭이면 `clean` 가능 |
